...
# The solution works, but there's an error in the expansion of the galaxy that's
# essentially off by one per column shifted.
#
# Abandoned once I realized manhattan distance is the simpler solution to the problem.
class Point
  include Comparable
  ...
  def shift_x(dark_matter, red_shift: 1)
    dx = dark_matter.select{|xx| xx <= x }
    @x += (dx.size * red_shift)
  end

  def shift_y(dark_matter, red_shift: 1)
    dy = dark_matter.select{|yy| yy <= y }
    @y += (dy.size * red_shift)
  end
...
class Wormhole
  ...
  def distance
    @distance ||= source.travel(destination) # (path.size - 2) #.tap{ |d| puts " => #{d}" }
  end
...
class Universe
  attr_reader :universe, :points, :galaxies, :space, :nodes, :dark_matter_x, :dark_matter_y, :red_shift

  def initialize(data, red_shift: 1)
    @universe = to_points(data)
    @red_shift = red_shift
    find_dark_matter(data)
  ...
  # The logic of expanding the universe doesn't quite work right.
  # when shifting columns, the count is off.
  def expand_universe
    galaxies.each do |galaxy|
      galaxy.shift_x(dark_matter_y, red_shift: red_shift)
      galaxy.shift_y(dark_matter_x, red_shift: red_shift)
    end
  end

  def find_dark_matter(data)
    find_dark_matter_x(data)
    find_dark_matter_y(data)
  end

  def find_dark_matter_x(rows)
    @dark_matter_x = rows[0].size.times.map do |x|
      x if rows.map{ |row| row[x] }.all?{ |pixel| pixel == '.' }
    end.compact
  end

  def find_dark_matter_y(rows)
    @dark_matter_y = rows.map.with_index do |row, y|
...
universe = Universe.new(data, red_shift: 1)
...
puts "dark matter: #{universe.dark_matter_x}, #{universe.dark_matter_y}"
# puts '=' * 40
# universe.universe.each do |row|
#   row.each do |point|
#     if point.galaxy?
